...
`gcloud`, `terraform`, `kubectl`, `aws` — is a symlink to a four-line script that
...
and never once asked your permission. Reads are auto-approved. "Nothing happens
without your approval" really means "nothing *destructive* happens without your
approval."
...
It can — and it is worth being clear about why. There is no sandbox in that
command at all. Those three flags limited which *tools* the model had; they said
nothing about which *files* those tools could open. Reads were bounded by nothing
except your own file permissions.

**Restricting tools is not restricting access.** That is what Step 2 fixes.
...
Type `/sandbox policy` and read it before you trust it. Then try the same request
that worked a minute ago:

> Read ../docs/answers.md

This time it should be refused.

> **`--experimental` is not optional.** `/sandbox` is only registered in
> experimental mode; without it, `/sandbox status` answers `Unknown command`.
> GitHub's web docs omit this — `copilot help sandbox` is the source of truth.
...
Same shape. Same fake binary behind it. Denied.
...
The read-only policy from Step 2 would block every edit, so swap it for the
read-write one — same file, one word different:
...
**A pattern is not an intent.** `:*` matches the command stem followed by a space,
so `shell(gcloud:*)` catches `gcloud storage ...` but *not* a bare `gcloud`. That
is why Step 3 denies both spellings. It also does not catch `bash -c 'gcloud ...'`,
a script the agent wrote first, or an MCP server calling the API directly.
...
- **What is verified and what is not:** the sandbox schema and behaviour quoted
  here were checked against Copilot CLI **1.0.83**. Deny precedence, the
  `--excluded-tools` interaction and the hook short-circuit come from GitHub's
  documentation and were not observed. Model behaviour varies — if your run differs,
  trust what you saw.
